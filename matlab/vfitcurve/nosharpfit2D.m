...
rxdb4=cxdb4;
rxdb4(sum(lxdb4(1:1+cutdb4level))+1:end) = 0;
iwxdb4 = waverec(rxdb4,lxdb4,'db4');

[cydb4,lydb4] = wavedec(P(:,2),leveldb4,'db4');
rydb4=cydb4;
rydb4(sum(lxdb4(1:1+cutdb4level))+1:end) = 0;
iwydb4 = waverec(rydb4,lydb4,'db4');

Pdb4=[iwxdb4 iwydb4];
Pdb4(end+1,:)=Pdb4(1,:);
energydb4=iwxdb4.^2+iwydb4.^2;
...
plot(Pdb4(:,1),Pdb4(:,2),'Color',[0 102 153]/255,'LineWidth',2)
...
ceV=cumsum(sort(energyV(5:max(range2cut)),'descend'));
normveV=ceV/ceV(end);
cedb4=cumsum(sort(energydb4(5:max(range2cut)),'descend'));
normvedb4=cedb4/cedb4(end);
% cedb2=cumsum(sort(energydb2(5:max(range2cut)),'descend'));
% normvedb2=cedb2/cedb2(end);
ceF=cumsum(sort(energyf(5:max(range2cut)),'descend'));
normveF=ceF/ceF(end);
cecoif3=cumsum(sort(energycoif3(5:max(range2cut)),'descend'));
normvecoif3=cecoif3/cecoif3(end);
cesym5=cumsum(sort(energysym5(5:max(range2cut)),'descend'));
...
%% 3D全局误差图
GNFerr3D(tzloc,errV,errf,errdb4,errcoif3,errsym5)
