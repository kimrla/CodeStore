function Lambda = fitjiaceng(P,k,N,t,u,CList)
...
%  
 ...
 %% 系数矩阵
 A0= LSMatrix_V(k,N,t);
 A=A0(u,size(A0,2)/2+1:end);
 %% 约束方程
 NumCns = sum(CList(:,3)+1);   % 约束方程数目
 NumBas = (k+1)*2^(N-1);       % 基函数数目
 C0 = zeros(NumCns,NumBas);     % 约束矩阵
 cidx = 1; 
 for h = 1 : size(CList,1)
     C0(cidx:cidx+CList(h,3),:) = derivativediff(k,N,CList(h,1),CList(h,2),CList(h,3));  % 计算0~CList(h,3)阶导数之差
     cidx = cidx + CList(h,3) + 1;
 end
 C=C0(:,NumBas/2+1:NumBas);
 %% KKT
 M = [2*A'*A,C';
      C,zeros(NumCns)];               % KKT方程系数矩阵
 b = [2*A'*P;zeros(NumCns,NumDim)] ;  % 
 Lambda0 = pinv(M)* b;
 Lambda = Lambda0(1:NumBas/2,:);
