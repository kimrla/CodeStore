% ***************************************************************
% *** Matlab function for finding gravity anomaly of fixed density distribution
...
function grav=poly_gravity(x_obs,z_obs,x,z,roh,t,c)

%poly_gravity function calculates z component of gravity field for any polygon 
...
%	roh   = the constant density contrast .
...
% direction, otherwise sign convention will create problem while running. 
    ...
    for i=1:length(x_obs) % Loop for all observation points. 
        for j=1:n_poly % Loop for line integral over all sides of polygon 
            ...
            ax=-2.*roh.*G.*(atan(ax1./ax2)).*(z(j+1)-z(j));
            value(j) = sum(c.*ax);
