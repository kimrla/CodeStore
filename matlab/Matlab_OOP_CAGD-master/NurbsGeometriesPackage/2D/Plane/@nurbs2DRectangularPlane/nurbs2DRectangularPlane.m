...
classdef nurbs2DRectangularPlane < nurbsGeometry
    
    methods
        function obj = nurbs2DRectangularPlane(varargin)
            [origin, width, height, angle, constructorData] = constructorPreprocessor(varargin{:});
            obj@nurbsGeometry(constructorData{1:2});
            constructorPostprocessor(obj, origin, width, height, angle, constructorData{:});
        end
...
%%
function [origin, width, height, angle, constructorData] = constructorPreprocessor(varargin)

if nargin == 0; origin = [0 0]; width = 1; height = 1;
...
constructorData = {[uKnotVector vKnotVector], controlPoints};

end


function constructorPostprocessor(obj, origin, width, height, angle, varargin)
