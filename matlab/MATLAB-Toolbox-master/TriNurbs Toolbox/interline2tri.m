...
if inter<tol*1e-10
    ...
    if d<tol*1e-10
        x(3,1)=0; pts2(1,:)=L(1,:); 
        [pts1(1,:), x(1:2,1)]=interpoint2tri(T, pts2(1,:)); 
        x(3,2)=1; pts2(2,:)=L(2,:); 
        [pts1(2,:), x(1:2,2)]=interpoint2tri(T, pts2(2,:)); 
        [tpts1, tpts2, tx, dk]=interline2line(T([1,2],:), L); % Edge 1 of the triangle
        t=2; 
        if dk<tol*1e-10
            tk=size(tx, 2); 
        ...
        [tpts1, tpts2, tx, dk]=interline2line(T([3,1],:), L); % Edge 2 of the triangle
        if dk<tol*1e-10
            tk=size(tx, 2); 
            pts1(t+1:t+tk,:)=tpts1(:,:); pts2(t+1:t+tk,:)=tpts2(:,:); 
        ...
        [tpts1, tpts2, tx, dk]=interline2line(T([3,2],:), L); % Edge 3 of the triangle
        if dk<tol*1e-10
            tk=size(tx, 2); 
            pts1(t+1:t+tk,:)=tpts1(:,:); pts2(t+1:t+tk,:)=tpts2(:,:); 
