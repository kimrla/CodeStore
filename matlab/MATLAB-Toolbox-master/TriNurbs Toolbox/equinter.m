function [npts,npnts1,npnts2,ndist]=equinter(pnts,tnrb,dist)
...
%设置曲线次数
% order=3;
% nrl1=nrlmake(pnts,pts1,[0,1],order);
...
dsrf=nrbderiv(srf);
for k=1:np
    [npts(k,:), npnts2(k,:), ndist(k)]=nrbsrfreverse(srf, dsrf, nodes(p2(k),:), npnts1(k,:)');%反求节点参数
end

%The fisrt point is the same as the last one
%如果首末点重合，通过nrbsrfreverse反求出来的参数点也是同一个点，要进行处理
