...
if inter<tol*1e-10
    ...
    pts1(1,:)=L1(1,:); x(1,1)=0;
    [pts2(1,:), x(2,1), d]=interpoint2line(L2, pts1(1,:));
    pts1(2,:)=L1(2,:); x(1,2)=1;
    [pts2(2,:), x(2,2)]=interpoint2line(L2, pts1(2,:));
    pts2(3,:)=L2(1,:); x(2,3)=0;
    [pts1(3,:), x(1,3)]=interpoint2line(L1, pts2(3,:));
    pts2(4,:)=L2(2,:); x(2,4)=1;
    [pts1(4,:), x(1,4)]=interpoint2line(L1, pts2(4,:));
    p=x>-eps & x<1+eps;
    p=p(1,:) & p(2,:);
    x=x(:,p);
    pts1=pts1(p,:);
...
% 
% % Plot the results
% Pts1(1,:)=(1-x(1,1))*L1(1,:)+x(1,1)*L1(2,:);
% Pts1(2,:)=(1-x(1,2))*L1(1,:)+x(1,2)*L1(2,:);
% Pts2(1,:)=(1-x(2,1))*L2(1,:)+x(2,1)*L2(2,:);
% Pts2(2,:)=(1-x(2,2))*L2(1,:)+x(2,2)*L2(2,:);
% figure; hold on;
% plot3(L1(:,1), L1(:,2), L1(:,3));
% plot3(L2(:,1), L2(:,2), L2(:,3));
% plot3(pts1(:,1), pts1(:,2), pts1(:,3), 'ro');
