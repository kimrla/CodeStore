...
for i=1:length(ed)
    d1=pp-repmat(pnts_now(ed(i,1),:),[length(pp),1]);
    d1=sqrt(sum(d1.*d1,2));
    d1=min(d1);
    d2=pp-repmat(pnts_now(ed(i,2),:),[length(pp),1]);
    d2=sqrt(sum(d2.*d2,2));
    d2=min(d2);   
    if min(d1,d2)>TOL
        tri1=p2t{ed(i,1)};
        ...
        if length(tri3)==2 && isempty(intersect(TRI,tri3))
            p3=setdiff(tri(tri3(1),:),ed(i,:));
            p4=setdiff(tri(tri3(2),:),ed(i,:));
            if abs(norm(pnts_now(ed(i,1),:)-pnts_now(ed(i,2),:))-norm(pnts_now(p3,:)-pnts_now(p4,:)))<TOL
